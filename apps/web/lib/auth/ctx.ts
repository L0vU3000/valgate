...
async function resolveCtx(): Promise<Ctx> {
  if (env.DEMO_MODE || process.env.STAGING_DEMO_MODE === "true") {
    ...
    // exist: production (no flag combination is exempt), or a real Clerk key configured
    // (DEMO_MODE left on by mistake). (M1, D9)
    // Staging preview exception: STAGING_DEMO_MODE=true bypasses the Clerk-key check only, for
    // Tailscale previews — it never re-opens the production path.
