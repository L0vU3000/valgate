...
final class APIRequestFactoryTests: XCTestCase {
    ...
    func test_listDocumentsRequest_hasExpectedPathAndMethod() {
        let factory = APIRequestFactory(baseURL: baseURL)

        let request = factory.urlRequest(for: .listDocuments(propertyId: "prop_42"), sessionToken: nil)

        XCTAssertEqual(request.url?.absoluteString, "https://staging.example.invalid/api/v1/properties/prop_42/documents")
        XCTAssertEqual(request.httpMethod, "GET")
    }

    func test_listLeasesRequest_hasExpectedPathAndMethod() {
        let factory = APIRequestFactory(baseURL: baseURL)

        let request = factory.urlRequest(for: .listLeases(propertyId: "prop_42"), sessionToken: nil)

        XCTAssertEqual(request.url?.absoluteString, "https://staging.example.invalid/api/v1/properties/prop_42/leases")
        XCTAssertEqual(request.httpMethod, "GET")
    }
    ...
    func test_ownershipRequest_hasExpectedPathAndMethod() {
        let factory = APIRequestFactory(baseURL: baseURL)

        let request = factory.urlRequest(for: .ownership(propertyId: "prop_42"), sessionToken: nil)

        XCTAssertEqual(request.url?.absoluteString, "https://staging.example.invalid/api/v1/properties/prop_42/ownership")
        XCTAssertEqual(request.httpMethod, "GET")
    }
